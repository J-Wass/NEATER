...
def test_flappy(individual):
    ...
    for f in range(frames):
        ...
        next_distance = state['next_next_pipe_dist_to_player']
        pl_height = state['player_y']
        pipe_height = state['next_pipe_top_y']
        jump = individual.activate([distance,next_distance, pl_height,pipe_height])[0]
        if jump < 0.5:
            flappy_game.act(JUMP)
        else:
...
if __name__ == '__main__':
    ...
    model = neat.NEATModel(config_file="flappy.config")
