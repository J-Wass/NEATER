...
if __name__ == '__main__':
    model = NEATModel(config_file="/home/jonah/Desktop/PLANET-master/projectile.config")
    best_genome = model.run(fitness_function=test_projectile_motion)
    print(best_genome)
    ans = best_genome.activate([2.0,3.0,3.0])
    print("Got {0},{1}, expected {2},{3}".format(ans[0], ans[1], 2*3, 2*3-9.8*0.5*(2**2)))
